...
// Send WebRTC logs to server - Now uses console only as the PHP endpoint has been archived
async function sendWebRTCLogs(sessionId, clientId) {
  if (!window.webrtcLogs) return;
  
  try {
    // Log to console instead of sending to server
    console.log(`WebRTC logs for session ${sessionId}, client ${clientId}:`, window.webrtcLogs);
    return {
      status: 'success',
      message: 'Debug logs have been output to the console. The server debug endpoint is no longer available.'
    };
      },
      body: JSON.stringify({
